...
module top_mgu #(
    ...
    logic signed [PRECISION:0] weights_lin_input [2*CHANNELS-1:0][0:0];
    initial begin
    ...
    logic signed [31:0] bias_lin_input [2*CHANNELS-1:0];
    initial begin
        bias_lin_input = {   -2264,   451,  4349, -6169,  9931,  6154, -9637, -5638,  4650, 10381,  -785,  7112, 10409,  
                        -151, -2732,  2143, 10208, -2451, 3275,  5265,  7406,  -813,  8268,  4836};
    end
    ...
    logic signed [PRECISION:0] weights_lin_hidden [2*CHANNELS-1:0][CHANNELS-1:0];
    initial begin
    ...
    logic signed [31:0] bias_lin_hidden [2*CHANNELS-1:0];
    initial begin
    ...
    logic [PRECISION-1:0]       lin_input  [(2*CHANNELS)-1:0];
    logic [PRECISION-1:0]       i_f  [CHANNELS-1:0];
    logic [PRECISION-1:0]       i_h  [CHANNELS-1:0];

    matrix_multiplication #(
        .INPUT_DIM      ( 1 ),
        .INPUT_PRECISION (16),
        .OUTPUT_DIM     ( 2*CHANNELS ),
        .PRECISION      ( PRECISION ),
    ...
    always @(posedge clk) begin // 1 cc
        i_f = lin_input[CHANNELS-1:0];
        i_h = lin_input[2*CHANNELS-1:CHANNELS];
    end
    ...
    logic [PRECISION-1:0]   lin_hidden  [2*CHANNELS-1:0];
   
    logic [PRECISION-1:0]   h_f  [CHANNELS-1:0];
    logic [PRECISION-1:0]   h_h  [CHANNELS-1:0];
    ...
    assign h_f = lin_hidden[CHANNELS-1:0];
    assign h_h = lin_hidden[2*CHANNELS-1:CHANNELS];
    ...
    logic [PRECISION:0] f_add  [CHANNELS-1:0];
    
    generate
        for (i = 0; i < CHANNELS; i = i + 1) begin : add_vector_f
    ...
    logic [PRECISION-1:0] f  [CHANNELS-1:0];
    generate
        for (i = 0; i < CHANNELS; i = i + 1) begin : lut_vector_f
    ...
    logic [PRECISION-1:0] hz_delay  [CHANNELS-1:0];
    
    logic [PRECISION-1:0]       f_hn  [CHANNELS-1:0];
    logic signed [PRECISION:0]  r_signed [CHANNELS-1:0];
    logic signed [PRECISION:0]  h_z_signed [CHANNELS-1:0];
    
    generate
        for (i=0; i<CHANNELS; i++) begin : signed_f
            assign r_signed[i] = {1'b0, f[i]};
        end
    endgenerate
    
    generate
        for (i=0; i<CHANNELS; i++) begin : signed_hz
            assign h_z_signed[i] = {1'b0, hz_delay[i]};
        end
    endgenerate
    ...
    generate
        for (i = 0; i < CHANNELS; i = i + 1) begin : sub_1_f
    ...
    generate
        for (i = 0; i < CHANNELS; i = i + 1) begin : delays_h_z
            delay_module #(
                .N        ( 8 ),
                .DELAY    ( 3 )
            ) delay_h_n_module (
