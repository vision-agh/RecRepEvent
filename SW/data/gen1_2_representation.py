...
class Gen1(L.LightningDataModule):
    ...
    def prepare_data(self):
        ...
        for split in ["train", "val", "test"]:
            input_path = os.path.join(self.cfg["data_dir"], f"{split}.h5")
            ...
            with h5py.File(input_path, "r") as in_h5, \
                ...
                events_splits = in_h5["events_splits"][:]
                ...
                for i in tqdm(range(len(events_splits) - 1), desc=f"Processing {split}"):
                    # slice event and bbox ranges
                    e_start, e_end = events_splits[i], events_splits[i + 1]
    ...
    def setup(self, stage=None):
        ...
        self.train_data = EventDatasetRepresentation(base / "train.h5", self.cfg)
        self.val_data   = EventDatasetRepresentation(base / "val.h5",   self.cfg)
        self.test_data  = EventDatasetRepresentation(base / "test.h5",  self.cfg)
