...
class Gen1(L.LightningDataModule):
    ...
    def prepare_data(self):
        if self.cfg["representation"] in ["Encoder", "TORE"]:
            ...
            for split in ["train", "val", "test"]:
                ...
                with h5py.File(input_path, "r") as in_h5, \
                    ...
                    rep_splits = in_h5["rep_splits"][:]
                    bboxes_splits = in_h5["bboxes_splits"][:]
                    ds_ev_full = in_h5["representations"]
                    ...
                    for i in tqdm(range(len(rep_splits) - 1), desc=f"Processing {split}"):
                        # slice event and bbox ranges
                        e_start, e_end = rep_splits[i], rep_splits[i + 1]
    ...
    def setup(self, stage=None):
        ...
        self.val_data   = EventDatasetRepresentation(base / "val.h5",   self.cfg, augment=False)
