...
class MyLiGRUCell(nn.Module):
    def __init__(self, 
                 input_size:int, 
                 hidden_size:int,
                 num_bits:int = 8):
        super(MyLiGRUCell, self).__init__()
        """
        Initialize the Minimal Gater Unit cell with input size and hidden size.
        Args:
        ...
        # MGU weights
        self.linear_input = nn.Linear(input_size, 2 * hidden_size, bias=False)
        self.linear_hidden = nn.Linear(hidden_size, 2 * hidden_size, bias=False)
    ...
    def forward(self, 
        ...
        # Reset gate
        gate_if = self.linear_input(x)
        gate_hf = self.linear_hidden(h)

        # Split the gates
        i_z, i_h = gate_if.chunk(2, 1)
        h_z, h_h = gate_hf.chunk(2, 1)

        z = torch.sigmoid( self.norm1(i_z) + h_z)
        h_prim = torch.relu( self.norm2(i_h) + h_h)

        # Final output
        new_h = z * h + (1 - z) * h_prim
        return new_h
