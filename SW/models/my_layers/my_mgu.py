...
class MyMGUCell(nn.Module):
    def __init__(self, 
                 input_size:int, 
                 hidden_size:int,
                 num_bits:int = 8):
        super(MyMGUCell, self).__init__()
        """
    ...
    def forward(self, 
        ...
        # Reset gate
        gate_if = self.linear_if(x)
        gate_hf = self.linear_hf(h)
        f = torch.sigmoid(gate_if + gate_hf)

        # Update gate
        gate_ih = self.linear_ih(x)
        gate_hh = self.linear_hh(f * h)
        h_prim = torch.tanh(gate_ih + gate_hh)

        # Final output
        new_h = (1 - f) * h + f * h_prim
        return new_h
